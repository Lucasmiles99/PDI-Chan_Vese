...
def apply_chan_vese(image, mu=0.2, lambda1=1, lambda2=1.5, tol=1e-3, max_iter=200):
    ...
    # Inicialização do level set - ajustado para forma de árvore
    height, width = image.shape
    center_x, center_y = width // 2, height // 2
    y, x = np.ogrid[:height, :width]
    # Ajusta a forma inicial para ser mais adequada a uma árvore
    phi = ((y - height*0.6)**2 / (height/2)**2 + 
           (x - center_x)**2 / (width/2)**2 - 1)
...
def main():
    ...
    plt.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))  # Convertendo BGR para RGB
    plt.axis('off')
    ...
    plt.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))  # Convertendo BGR para RGB
    plt.contour(phi, [0], colors='red', linewidths=2)
    plt.axis('off')
